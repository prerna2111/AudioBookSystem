...
typedef struct Preference_tag                //This is a structure for creating a lists of preferences for each user
{
    char pref[MAX_SIZE];
    struct Preference_tag *prefnext;
} Preference;

typedef struct Timestamp_tag                //structure defined for reading time in case of duration and timestamp
{
    int h;
    int m;
    int s;
...
typedef struct audio_tag                  //structure defined for the main library
{
    int AudiobookID;
    char Title[MAX_SIZE];
...
Audio *Mainlist;                         //this is the list which contains all the audiobooks
user *Userlist;                          // this is the list which contains all the users
...
void CreateMainList()                                       //function for creating the main library
{
    Mainlist = NULL; 
    ...
    float rating;
    // char line[MAX_SIZE];
    while (fgets(line, sizeof(line), fptr) != NULL)                 //used to read audiobooks from the csv file
    {
    ...
    fclose(fptr);
    Mainlist = mergesort_AudioID(Mainlist);                     //the mainlist is to be sorted on the basis of audiobook ID
}

Audio *merge(Audio *lptr1, Audio *lptr2)                 //Merge sort algo for audiobooks
{
...
{
    Audio *fast, *slow, *ptr;              //slow-fast approach to divide the list and merge them accordingly
    slow = lptr;
...
void Add_Audiobook()                                    //function to add audiobooks to the mainlist
{
    ...
    if (Mainlist == NULL || Mainlist->AudiobookID > nptr->AudiobookID)        //handles the case where The mainlist is empty
    {
        nptr->audionext = Mainlist;
        Mainlist = nptr;
    }
    ...
    {
        // printf("\nelse");
        while (ptr != NULL && ptr->AudiobookID < nptr->AudiobookID)       //if mainlist is not empty traversal is done to find the correct position on the basis of audiobook
        {
            prev = ptr;
            ptr = ptr->audionext;
        }
...
void Edit_Audiobook()             //function to edit audiobbok
{
    int id;
    ...
    Audio *ptr;
    ptr = Mainlist;
    while (ptr->AudiobookID != id)                 //traversal done to find the respective ID
    {
        ptr = ptr->audionext;
    }
    if (choice != 4)
...
void Delete_Audiobook()                                     //function to delete an audiobook
{
    ...
    while (ptr != NULL)
    {
        if (strcmp(ptr->Title, str) == 0 || strcmp(ptr->Author, str) == 0)            //the respective audiobook is found by traversal by comparing strings of Title and Author
        {
            id = ptr->AudiobookID;
            if (prev == NULL)                 //if the audiobook to be deleted is the first one
            {
    ...
    if (prev->audionext == NULL)             //if audiobook is not present
    {
    ...
    else
    {
        ...
        while (nptr != NULL)                    //the audiobook is here being deleted from users' list also
        {
            printf("check");
            userAudio *aptr = nptr->audiolist;
            userAudio *prev = NULL;
            int flag = 0;
            while (aptr != NULL && flag == 0)           //traversal to find the audiobook from the list of User and delete from there 
            {
                if (aptr->userA_ID == id)
                {
                    if (prev == NULL && aptr != NULL)          //if the audiobook to be deleted is placed at first itself
                    {
...
void Search_Audiobook()               //function to search an audiobook and display the respective details
{
    ...
    while (ptr != NULL)
    {
        if ((strcmp(ptr->Title, str) == 0) || (strcmp(ptr->Author, str) == 0 || (strcmp(ptr->genre, str) == 0)))        //traversal to search the audiobook
        {
            printf("id=%d\n", ptr->AudiobookID);
            printf("title=%s\n", ptr->Title);
            printf("\n");
...
userAudio *Add_userAudio(userAudio *ual, userAudio *temp)      //Helper function to add a user to the Userlist which traverses the list and returns the desired pointer so that userId remains sorted
{ 
    if (ual == NULL)
    {
        ual = temp;
...
userAudio *merge_UaudioID(userAudio *lptr1, userAudio *lptr2)    //mergesort function for sorting the users based on their User ID
{
...
void CreateUserList()            //function to create a list of Users
{
    ...
    fclose(fptr);
    Userlist = mergesort_Userlist(Userlist);              //the Userlist is sorted based on the User ID
}
...
void delete_user_list(user *head)              //funcion to free the memory the userlist is consuming
{
    user *current = head;
    user *next;
...
Audio *return_audioptr_of_Main(int aid)                //this function is used to return pointer of the Audio from the Mainlist based on the ID
{
    Audio *ptr = Mainlist;
...
user *return_userptr(int uid)                  //this function is used to return pointer of the Audio from the Userlist based on the ID
{
    user *ptr = Userlist;
...
user *Add_user_profile(user *Userlist)           //this function is used to add a user to the Userlist
{
...
user *Add_to_user_library(user *Userlist)                           //this function is for adding an audiobook to the audiolist of a particular user
{
    int uid, n, aid;
    ...
    case 1:                          //for adding audiobook
    {
        ...
        u->audiolist = Add_userAudio(u->audiolist, temp_book);                 //made use of the helper function
        break;
    }
    case 2:
    {
        printf("Removing audiobook\n");                //for removing an audiobook
        userAudio *ptr, *prev;
        prev = NULL;
        ptr = u->audiolist;
        while (ptr != NULL && ptr->userA_ID != aid)             //the audiobook is searched for that particular ID           
        {
            prev = ptr;
            ptr = ptr->userA_next;
        }
...
int compare_timestamp(Timestamp t1, Timestamp t2)                 //function for comparing timestamp
{
    int t;
    if (t1.h > t2.h)
    {
...
userAudio *merge_UaudioTimestamp(userAudio *lptr1, userAudio *lptr2)        //used for performing mergesort operation for sorting the users' audiolist on the basis of timestamp
{
    userAudio *ptr1, *ptr2, *result, *tail;
    if (lptr2 == NULL)
    {
...
void ListeningProgress()                            //function for tracking the listening progress of a particular user and updating it in user's library
{
    int uid, aid, h, m, s;
    ...
    userAudio *audioptr = ptr->audiolist;
    while (audioptr != NULL && audioptr->userA_ID != aid)           //traversal to find the audio in user's library
    {
        audioptr = audioptr->userA_next;
    }
    ...
    if (audioptr->timestamp.s >= 60)                //care has been taken of the fact if the seconds' part in timestamp becomes>60
    {
...
void Display_User_Library()             //function for displaying user library sorted on the basis of timestamp
{
    int uid;
    char str[MAX_SIZE];
    ...
    uptr->audiolist = mergesort_timestamp(uptr->audiolist);     //used to perform mergesort operation on the basis of timestamp
    ...
    uptr->audiolist = mergesort_UaudioID(uptr->audiolist);             //list is sorted on the basis of audiobook ID in the mainlist
}

void set_AverageRating(Audio *nptr)              //This is used to calculate average rating of a particular audiobook by traversing the userlist and mainting the count and sum of rating
{
    user *uptr = Userlist;
    int count = 0;
    float sum = 0;
...
void Rate_Audiobook()                         //This function allows user to rate an audiobook
{
    int uid, aid;
    float rate;
    printf("Enter userid:");
    ...
    scanf("%d", &aid);
    printf("Enter the rating you want to give for the Audiobook(rate between 1-5):");
    scanf("%f", &rate);
...
void Most_popular_Audiobook()         //this function is used to return the most popular audiobook by analyzing the average rating of all audiobooks from the Mainlist
{
    Audio *ptr = Mainlist;
...
void Listening_history_report_user()             //this function is used to print the listening report of the user and printing the total listening time of the user
{
    int uid;
    Timestamp total;
...
void Listening_history_report_audiobook()              //this is used to print the listening report of a particular audiobook 
{
    int aid;
    Timestamp total;
    total.h = total.m = total.s = 0;
...
void deleteLinkedList()                         //this is used to free the memory of the Mainlist 
{
...
int compareTitle(userAudio *ptr1, userAudio *ptr2)              //This function is used to compare the title of 2 audiobooks
{
    int ret_val = 0;
...
void MergeUserLibraries()            //This function helps in merging audiobook libraries of two users
{
    int uid1, uid2;
